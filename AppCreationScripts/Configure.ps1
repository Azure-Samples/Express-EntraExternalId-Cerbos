...
<#
 This script creates the Microsoft Entra ID applications needed for this sample and updates the configuration files
 for the visual Studio projects from the data in the Microsoft Entra ID applications.
...
{
    <#.Description
       This function creates the Microsoft Entra ID applications for the sample in the provided Microsoft Entra ID tenant and updates the
   ...
   Write-Host "Creating the AAD application (ciam-msal-node-webapp)"
   # Get a 6 months application key for the client Application
   $fromDate = [DateTime]::Now;
   $key = CreateAppKey -fromDate $fromDate -durationInMonths 6
   
   # create the application 
   $clientAadApplication = New-MgApplication -DisplayName "ciam-msal-node-webapp" `
                                                      -Web `
                                                      @{ `
                                                          RedirectUris = "http://localhost:3000", "http://localhost:3000/auth/redirect", "http://localhost:3000/redirect"; `
    ...
    #Update-MgApplication -ApplicationId $currentAppObjectId -IdentifierUris @("https://$tenantName/ciam-msal-node-webapp")
    ...
    Write-Host "Done creating the client application (ciam-msal-node-webapp)"
    ...
    Add-Content -Value "<tr><td>client</td><td>$currentAppId</td><td><a href='$clientPortalUrl'>ciam-msal-node-webapp</a></td></tr>" -Path createdApps.html
    ...
    # Define a new Roles
    $rolesList = New-Object System.Collections.Generic.List[Microsoft.Graph.PowerShell.Models.IMicrosoftGraphAppRole]

    # Define a new Admin role
    $newAdminRole = @{
        AllowedMemberTypes = "User"
        Description = "Admins can manage all posts"
        DisplayName = "Admin"
    ...
    # Add the new Admin role to the list
    $rolesList.Add($newAdminRole)

    # Define a new User role
    $newUserRole = @{
        AllowedMemberTypes = "User"
        Description = "User can manage own posts"
        DisplayName = "User"
        Id = (New-Guid).ToString()
        IsEnabled = $true
        Value = "Posts.User"
    ...
    # Add the new User role to the list
    $rolesList.Add($newUserRole)

    # Update the application with the new roles
    Update-MgApplication -ApplicationId $currentAppObjectId -AppRoles $rolesList
    ...
    $dictionary = @{ "Enter_the_Application_Id_Here" = $clientAadApplication.AppId; "Enter_the_Tenant_Subdomain_Here" = $tenantName.Split(".onmicrosoft.com")[0]; "Enter_the_Client_Secret_Here" = $clientAppKey };
